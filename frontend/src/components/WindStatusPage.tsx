...
export default function WindStatusPage({ fiscalYear }: { fiscalYear: string }) {
    ...
    const { data: rawProjects = [], isLoading: projectsLoading } = useQuery<CommissioningProject[]>({
        ...
        queryFn: async () => {
            const response = await fetch(`${API_BASE}/api/commissioning-projects?fiscalYear=${fiscalYear}&category=wind`);
    ...
    // Deduplicate and filter for Wind only + APPLY DATE VERIFICATION
    const windProjects = useMemo(() => {
        const seen = new Set();
        const windOnly = scopeFilteredProjects.filter((p: any) => {
            const cat = (p.category || '').toLowerCase();
            const name = (p.projectName || '').toLowerCase();
            const isWind = (cat.includes('wind') || name.includes('wind')) && !cat.includes('solar') && !name.includes('solar');
            if (!isWind) return false;
    ...
    const yearlyData = useMemo(() => {
        ...
        return [{
            ...
            Plan: planProjects.reduce((s, p) => 
                s + planMonths.reduce((sum, m) => sum + ((p as any)[m.key] || 0), 0), 0
            ),
            'Actual/Fcst': actualProjects.reduce((s, p) => s + (p.totalCapacity || 0), 0),
        }];
    }, [overallFilteredProjects, fiscalYear, currentFY, _monthsPassed, ALL_MONTHS]);
    ...
    const handleExportDetailed = () => {
        ...
        XLSX.writeFile(wb, `AGEL_Wind_Detailed_${fiscalYear}_${new Date().toISOString().split('T')[0]}.xlsx`);
    };
    ...
    if (projectsLoading) {
        return <PageLoader message="Loading Wind Dashboard..." />;
    }
    ...
    return (
        <div className="space-y-6 min-h-screen">
            ...
            <motion.div
                ...
                className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#0B74B0] via-[#075985] to-[#1e3a5f] p-8 shadow-2xl"
            >
                {/* Animated Background Elements */}
                <div className="absolute inset-0 overflow-hidden">
                    {/* Flowing wind lines */}
                    <motion.div
                        className="absolute -right-10 -top-10 w-64 h-64 bg-cyan-400/15 rounded-full blur-3xl"
                        animate={{
                            scale: [1, 1.2, 1],
                            opacity: [0.1, 0.2, 0.1],
                            x: [0, 30, 0],
                    ...
                    />
                    <motion.div
                        className="absolute -left-20 bottom-0 w-80 h-80 bg-teal-400/10 rounded-full blur-3xl"
                        animate={{
                            scale: [1, 1.3, 1],
                    ...
                    />
                    <motion.div
                        className="absolute right-1/3 top-1/2 w-48 h-48 bg-sky-300/15 rounded-full blur-2xl"
                        animate={{
                            scale: [1, 1.4, 1],
                            opacity: [0.1, 0.2, 0.1],
                            x: [0, 50, 0],
                    ...
                    {/* Wind turbine icon accent */}
                    <motion.div
                        className="absolute right-8 top-8 opacity-20"
                        animate={{ rotate: 360 }}
                        transition={{ duration: 8, repeat: Infinity, ease: "linear" }}
                    >
                        <svg className="w-24 h-24 text-cyan-300" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M12 2C12.5523 2 13 2.44772 13 3V10.5858C13.5978 10.7905 14.1135 11.1587 14.4854 11.6458L19.7782 9.77746C20.2983 9.59364 20.8671 9.86576 21.0509 10.3859C21.2347 10.906 20.9626 11.4748 20.4425 11.6586L15.1497 13.5269C15.0529 14.0057 14.8401 14.4439 14.5374 14.8096L17.5528 19.4472C17.8343 19.9412 17.6784 20.5771 17.1844 20.8588C16.6904 21.1405 16.0545 20.9845 15.7728 20.4905L12.7574 15.853C12.509 15.9496 12.2405 16 12 16C11.7595 16 11.491 15.9496 11.2426 15.853L8.22723 20.4905C7.94551 20.9845 7.30963 21.1405 6.81564 20.8588C6.32165 20.5771 6.16572 19.9412 6.44744 19.4472L9.46278 14.8096C9.15994 14.4439 8.94714 14.0057 8.85028 13.5269L3.55744 11.6586C3.03736 11.4748 2.76525 10.906 2.94906 10.3859C3.13287 9.86576 3.70174 9.59364 4.22182 9.77746L9.51466 11.6458C9.88653 11.1587 10.4022 10.7905 11 10.5858V3C11 2.44772 11.4477 2 12 2Z" />
                        </svg>
                    </motion.div>
                    {/* Flowing wind lines */}
                    <motion.div
                        className="absolute left-0 top-1/4 h-px w-full bg-gradient-to-r from-transparent via-white/20 to-transparent"
                        animate={{ x: [-200, 200], opacity: [0, 0.5, 0] }}
                        transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
                    />
                    <motion.div
                        className="absolute left-0 top-2/3 h-px w-full bg-gradient-to-r from-transparent via-white/15 to-transparent"
                        animate={{ x: [-100, 300], opacity: [0, 0.4, 0] }}
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div className="text-white">
                            ...
                            >
                                ...
                                <span className="text-xs font-bold uppercase tracking-[0.2em] text-white/70">Wind Energy</span>
                            </motion.div>
                            <motion.h1
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8 relative z-10">
                    ...
                    >
                        ...
                        <div className="mt-3 flex items-center gap-2">
                            <span className="px-2 py-1 bg-cyan-500/40 rounded-lg text-[10px] font-bold text-cyan-200 uppercase tracking-wider">
                    ...
                    >
                        ...
                        <div className="mt-3 flex items-center gap-2">
                            <span className="px-2 py-1 bg-teal-500/40 rounded-lg text-[10px] font-bold text-teal-200 uppercase tracking-wider">
                ...
                >
                    ...
                    <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 mt-6">
                        {sectionData.map((d, index) => (
                            <div key={index} className={`flex items-center gap-1.5 transition-all ${sectionHovered && sectionHovered.name !== d.name ? 'opacity-30' : 'opacity-100'}`}>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                        ...
                        <div className="flex bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-1">
                            <button
                                ...
                                className={`px-3 py-1.5 text-[10px] font-bold uppercase rounded-md transition-all ${viewMode === 'yearly' ? 'bg-[#0B74B0] text-white shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                            >
                                Yearly
                            </button>
                            <button
                                onClick={() => setViewMode('quarterly')}
                                className={`px-3 py-1.5 text-[10px] font-bold uppercase rounded-md transition-all ${viewMode === 'quarterly' ? 'bg-[#0B74B0] text-white shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                            >
                                Quarterly
                            </button>
                            <button
                                onClick={() => setViewMode('monthly')}
                                className={`px-3 py-1.5 text-[10px] font-bold uppercase rounded-md transition-all ${viewMode === 'monthly' ? 'bg-[#0B74B0] text-white shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                            >
                                Monthly
                            </button>
                        </div>
            ...
            >
                <div className="p-4 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 border-b border-gray-200 dark:border-gray-700 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <h3 className="text-lg font-bold text-gray-800 dark:text-white flex items-center gap-2">
                        ...
                        AGEL OVERALL WIND FY {fiscalYear.replace('FY_', '').split('-').map((p, i) => i === 0 ? '20' + p : p).join('-')}
                    </h3>
                ...
                <div className="flex flex-wrap items-center gap-6">
                    <div className="flex items-center gap-3">
                        <select
                            ...
                            className="px-4 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg text-sm font-black uppercase text-cyan-600 tracking-widest transition-all focus:ring-2 focus:ring-cyan-500/20"
                        >
                            {WIND_SECTIONS.map(s => (
                    ...
                    <div className="flex items-center gap-4 border-l border-gray-200 dark:border-gray-700 pl-6">
                        <div className="flex flex-col gap-0.5">
                            ...
                            <select
                                ...
                                className="px-2 py-1 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded text-sm font-bold text-gray-700 outline-none focus:ring-1 focus:ring-cyan-500/20"
                            >
                    ...
                    <div className="flex items-center gap-6 ml-auto">
                        ...
                        <div className="flex items-center gap-2">
                            <span className="w-3 h-3 rounded-full bg-[#75479C] shadow-sm shadow-emerald-500/50"></span>
            ...
            >
                <div className="p-4 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-cyan-50 to-teal-50 dark:from-gray-800 dark:to-gray-800 flex justify-between items-center">
                    <h3 className="text-lg font-bold text-gray-800 dark:text-white flex items-center gap-2">
                        ...
                        {selectedSection !== 'all' ? WIND_SECTIONS.find(s => s.value === selectedSection)?.label : 'Wind Projects - Plan / Actual Comparison'}
                    </h3>

                    <button
                        onClick={handleExportDetailed}
                        className="flex items-center gap-2 px-3 py-1.5 bg-cyan-600 hover:bg-cyan-700 text-white rounded-lg text-xs font-bold transition-all shadow-sm active:scale-95"
                    >
                ...
                <div className="overflow-x-auto max-h-[600px] scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-gray-800">
                    <table className="w-full text-sm border-collapse">
                        <thead className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 sticky top-0 z-30">
                            <tr>
                                <th className="px-4 py-4 text-center text-[11px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-wider w-14 border-b-2 border-dashed border-gray-300 dark:border-gray-600 sticky left-0 z-40 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900">No</th>
                                ...
                                <th className="px-4 py-4 text-center text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[100px] border-b-2 border-dashed border-gray-300 dark:border-gray-600">Status</th>
                                {visibleMonths.map(m => (
                                ...
                                <th className="px-4 py-4 text-right text-[11px] font-black text-cyan-600 dark:text-cyan-400 uppercase tracking-wider border-b-2 border-dashed border-cyan-500/30 dark:border-cyan-500/40 bg-cyan-500/5 dark:bg-cyan-500/10">Total</th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <React.Fragment key={`${group.projectName}-${idx}`}>
                                    ...
                                    {(capacityPointFilter === 'all' || capacityPointFilter === 'Plan') && (
                                        <tr className="group hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                                            <td className="px-4 py-3 text-gray-400 font-bold text-xs text-center border-r border-gray-50 dark:border-gray-800 sticky left-0 z-20 bg-inherit" rowSpan={capacityPointFilter === 'all' ? 2 : 1}>{idx + 1}</td>
                                            <td className="px-4 py-3 font-bold text-gray-800 dark:text-white sticky left-12 z-20 bg-inherit shadow-[4px_0_10px_rgba(0,0,0,0.02)]" rowSpan={capacityPointFilter === 'all' ? 2 : 1}>
                                                <span>{group.projectName}</span>
                                            </td>
                                            <td className="px-4 py-3 text-gray-500 dark:text-gray-400 font-medium text-xs truncate max-w-[140px]" rowSpan={capacityPointFilter === 'all' ? 2 : 1} title={group.spv}>{group.spv || '-'}</td>
                                            <td className="px-4 py-3 text-gray-600 dark:text-gray-400 text-[10px] font-bold uppercase" rowSpan={capacityPointFilter === 'all' ? 2 : 1}>
                                            ...
                                            <td className="px-4 py-3 text-gray-500 dark:text-gray-400 font-medium text-[10px]" rowSpan={capacityPointFilter === 'all' ? 2 : 1}>{group.plotLocation || '-'}</td>
                                            <td className="px-4 py-3 text-right font-black text-gray-900 dark:text-white" rowSpan={capacityPointFilter === 'all' ? 2 : 1}>{formatNumber(group.capacity)} <span className="text-[9px] font-bold opacity-30">MW</span></td>
                                            <td className="px-4 py-3 text-center border-l border-gray-50 dark:border-gray-800">
                                                <span className="inline-flex px-2.5 py-1 text-[9px] font-black uppercase rounded-lg bg-[#0B74B0]/10 text-[#0B74B0] dark:bg-[#0B74B0]/30 dark:text-[#60a5fa] shadow-sm">Plan</span>
                                            </td>
                                            {visibleMonths.map(m => (
                                                <td key={m.key} className="px-4 py-3 text-right tabular-nums border-r border-gray-100 dark:border-gray-800 text-gray-700 dark:text-gray-300 font-medium whitespace-nowrap text-xs">
                                            ...
                                            <td className="px-4 py-3 text-right font-black text-[#0B74B0] dark:text-[#60a5fa] bg-[#0B74B0]/10 dark:bg-[#0B74B0]/20">{formatNumber(group.plan?.totalCapacity)}</td>
                                        </tr>
                                    )}
                                    {/* Actual Row */}
                                    {(capacityPointFilter === 'all' || capacityPointFilter === 'Actual') && (
                                        <tr className="group hover:bg-[#75479C]/5 dark:hover:bg-[#75479C]/10 transition-colors bg-white dark:bg-gray-900 border-b-2 border-gray-100 dark:border-gray-800">
                                            {capacityPointFilter !== 'all' && (
                                                <>
                                                    <td className="px-4 py-3 text-gray-400 font-bold text-xs text-center border-r border-gray-50 dark:border-gray-800 sticky left-0 z-20 bg-inherit" rowSpan={1}>{idx + 1}</td>
                                                    <td className="px-4 py-3 font-bold text-gray-800 dark:text-white sticky left-12 z-20 bg-inherit shadow-[4px_0_10px_rgba(0,0,0,0.02)]" rowSpan={1}>
                                                        <span>{group.projectName}</span>
                                                    </td>
                                                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400 font-medium text-xs truncate max-w-[140px]" rowSpan={1} title={group.spv}>{group.spv || '-'}</td>
                                                    <td className="px-4 py-3 text-gray-600 dark:text-gray-400 text-[10px] font-bold uppercase" rowSpan={1}>
                                                        <span className={`px-2 py-0.5 rounded-md ${group.projectType?.toLowerCase().includes('ppa') ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'}`}>
                                                            {group.projectType || '-'}
                                                        </span>
                                                    </td>
                                                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400 font-medium text-[10px]" rowSpan={1}>{group.plotLocation || '-'}</td>
                                                    <td className="px-4 py-3 text-right font-black text-gray-900 dark:text-white" rowSpan={1}>{formatNumber(group.capacity)} <span className="text-[9px] font-bold opacity-30">MW</span></td>
                                                </>
                                            )}
                                            <td className="px-4 py-3 text-center border-l border-gray-50 dark:border-gray-800">
                                                <span className="inline-flex px-2.5 py-1 text-[9px] font-black uppercase rounded-lg bg-[#75479C]/10 text-[#75479C] dark:bg-[#75479C]/30 dark:text-[#d8b4fe] shadow-sm">Actual</span>
                                            </td>
                                            {visibleMonths.map(m => (
                                                <td key={m.key} className="px-4 py-3 text-right tabular-nums border-r border-gray-100 dark:border-gray-800 text-gray-700 dark:text-gray-300 font-medium whitespace-nowrap text-xs">
                                            ...
                                            <td className="px-4 py-3 text-right font-black text-[#75479C] dark:text-[#d8b4fe] bg-[#75479C]/5 dark:bg-[#75479C]/10">{formatNumber(group.actual?.totalCapacity)}</td>
                                        </tr>
                                    )}
                                </React.Fragment>
                            ))}
                        ...
                        >
                            ...
                            >
                                ...
                                <div className="p-6">
                                        ...
                                        <div className="flex flex-col gap-2">
                                            ...
                                            <div className="flex gap-2">
                                                <select
                                                    ...
                                                    className="flex-1 px-3 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm font-bold text-gray-700 dark:text-white focus:ring-2 focus:ring-cyan-500/20 outline-none transition-all cursor-pointer"
                                                >
                                    ...
                                    <div className="mt-6 p-4 bg-cyan-50 dark:bg-cyan-900/10 rounded-xl border border-cyan-100 dark:border-cyan-900/20 flex items-center justify-between">
                                        <div className="flex flex-col">
                                            <span className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Selection</span>
                                            <span className="text-sm font-black text-cyan-600 dark:text-cyan-400 mt-0.5">
